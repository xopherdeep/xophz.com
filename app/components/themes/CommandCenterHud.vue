...
<template>
  <section class="hud w-full min-h-[calc(100dvh-152px)] flex items-center justify-center px-4 py-8" aria-label="Home">
    <div class="relative w-full max-w-[1100px] min-h-[500px] flex items-center justify-center">
      ...
      <!-- Stats card (top-left) -->
      <div class="hidden md:flex absolute top-0 left-0 flex-col gap-2 p-4 bg-white/5 border border-white/12 rounded-2xl backdrop-blur-xl shadow-glass-shadow w-[200px] transition-all duration-300 hover:border-accent/25 hover:bg-white/8" :style="parallax(-6)" v-motion="{ initial: { opacity: 0, x: -30, y: -20 }, enter: { opacity: 1, x: 0, y: 0, transition: { delay: 200 } } }">
        <p class="text-[0.55rem] font-bold tracking-[0.18em] uppercase text-text-muted">STATS</p>
      ...
      <!-- Specialties card (top-right) -->
      <div class="hidden md:flex absolute top-4 right-0 flex-col gap-2 p-4 bg-white/5 border border-white/12 rounded-2xl backdrop-blur-xl shadow-glass-shadow w-[220px] transition-all duration-300 hover:border-accent/25 hover:bg-white/8" :style="parallax(-8)" v-motion="{ initial: { opacity: 0, x: 30, y: -20 }, enter: { opacity: 1, x: 0, y: 0, transition: { delay: 300 } } }">
        <p class="text-[0.55rem] font-bold tracking-[0.18em] uppercase text-text-muted">WHAT I DO</p>
        <div class="flex flex-col gap-1.5">
          <div v-for="s in specialties" :key="s.key" class="flex items-center gap-2 py-1">
            <component :is="s.icon" class="w-3.5 h-3.5 text-accent-2 shrink-0" />
            <span class="text-[0.72rem] text-text-secondary">{{ s.title }}</span>
          </div>
        </div>
      </div>

      <!-- Featured projects card (bottom-left) -->
      <div class="hidden md:flex absolute bottom-0 left-4 flex-col gap-2 p-4 bg-white/5 border border-white/12 rounded-2xl backdrop-blur-xl shadow-glass-shadow w-[230px] transition-all duration-300 hover:border-accent/25 hover:bg-white/8" :style="parallax(-5)" v-motion="{ initial: { opacity: 0, x: -30, y: 20 }, enter: { opacity: 1, x: 0, y: 0, transition: { delay: 400 } } }">
      ...
      <div class="hidden md:flex absolute bottom-4 right-0 flex-col gap-2 p-4 bg-white/5 border border-white/12 rounded-2xl backdrop-blur-xl shadow-glass-shadow w-[200px] transition-all duration-300 hover:border-accent/25 hover:bg-white/8" :style="parallax(-7)" v-motion="{ initial: { opacity: 0, x: 30, y: 20 }, enter: { opacity: 1, x: 0, y: 0, transition: { delay: 500 } } }">
        ...
        <div class="flex flex-col gap-1">
          <a v-for="link in featuredLinks" :key="link.id" :href="link.href" target="_blank" rel="noopener noreferrer" class="flex items-center gap-2 py-1.5 text-text-secondary no-underline transition-colors duration-200 hover:text-text-primary" :style="{ '--lc': link.color }">
            ...
            <span class="text-[0.72rem]">{{ link.label }}</span>
          </a>
        </div>
      </div>
      ...
      <div class="flex md:hidden absolute -bottom-6 left-0 right-0 gap-2 justify-center px-2">
        ...
        <NuxtLink
          to="/projects"
          class="flex-1 flex items-center justify-center gap-1.5 px-3.5 py-2.5 rounded-xl bg-white/5 border border-white/12 text-text-secondary no-underline text-xs font-semibold backdrop-blur-xl"
        >
