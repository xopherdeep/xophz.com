  ...
  <section v-if="hasIdentity" class="m-about-hero py-6">
    <UContainer class="max-w-[1100px] w-full">
      ...
      >
        <template #avatar>
          <div
            class="relative p-[3px] rounded-full bg-gradient-hero animate-[spinGlow_6s_linear_infinite] shrink-0 shadow-2xl"
        ...
        <template #meta>
          <span class="flex items-center gap-1 text-[0.68rem] font-semibold tracking-[0.1em] uppercase text-zinc-400 dark:text-zinc-500">
        ...
        <template #subtitle>
          <p class="text-sm sm:text-base font-semibold text-violet-600 dark:text-violet-400 mt-1">
            {{ profile.title }}
          </p>
        </template>

        <template #description>
          <p class="text-sm md:text-base leading-relaxed text-zinc-600 dark:text-zinc-300 mt-2">
            {{ identity.tagline }}
          </p>
        </template>
      </OPageHero>
